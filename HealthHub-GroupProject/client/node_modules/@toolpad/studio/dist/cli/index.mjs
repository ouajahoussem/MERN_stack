...
var DataManager = class {
  ...
  async setConnectionParams(connectionId, params) {
    ...
    const existing = appDom3.getNode(dom, connectionId, "connection");
    dom = appDom3.setNodeNamespacedProp(
      dom,
      existing,
      "attributes",
      "params",
      appDom3.createSecret(params)
    );
  ...
  async execQuery(pageName, queryName, params) {
    ...
    const dataNode = appDom3.getQueryByName(dom, page, queryName);
    if (!dataNode) {
...
function mergeThemeIntoAppDom(dom, themeFile) {
  ...
  const app = appDom4.getApp(dom);
  dom = appDom4.addNode(
    dom,
    appDom4.createNode(dom, "theme", {
...
function mergeApplicationIntoDom(dom, applicationFile) {
  ...
  const app = appDom4.getApp(dom);
  dom = appDom4.setNodeNamespacedProp(dom, app, "attributes", "plan", applicationFileSpec?.plan);
  dom = appDom4.setNodeNamespacedProp(dom, app, "attributes", "authentication", {
    ...applicationFileSpec?.authentication
  });
  dom = appDom4.setNodeNamespacedProp(dom, app, "attributes", "authorization", {
...
function expandChildren(children, dom) {
  return children.sort((child1, child2) => {
    ...
    return appDom4.compareFractionalIndex(child1.parentIndex, child2.parentIndex);
...
function expandFromDom(node, dom) {
  if (appDom4.isPage(node)) {
    const children = appDom4.getChildNodes(dom, node);
    return {
  ...
  if (appDom4.isElement(node)) {
    const { children, ...templates } = appDom4.getChildNodes(dom, node);
...
function mergeElementIntoDom(dom, parent, parentProp, elm) {
  ...
  const elmNode = appDom4.createElement(dom, elm.component, plainProps, elm.layout ?? {}, elm.name);
  dom = appDom4.addNode(dom, elmNode, parent, parentProp);
  if (elm.children) {
...
function createPageDomFromPageFile(pageName, pageFile) {
  ...
  let fragment = appDom4.createFragment("page", {
    name: pageName,
    attributes: {
  ...
  const pageNode = appDom4.getRoot(fragment);
  appDom4.assertIsPage(pageNode);
  if (pageFileSpec.queries) {
    for (const query of pageFileSpec.queries) {
      if (query.query) {
        const queryNode = appDom4.createNode(fragment, "query", {
          name: query.name,
          attributes: {
            connectionId: null,
        ...
        fragment = appDom4.addNode(fragment, queryNode, pageNode, "queries");
      }
    }
  }
...
function mergePageIntoDom(dom, pageName, pageFile) {
  const appRoot = appDom4.getRoot(dom);
  const pageFragment = createPageDomFromPageFile(pageName, pageFile);
  const newPageNode = appDom4.getRoot(pageFragment);
  if (appDom4.getMaybeNode(dom, newPageNode.id)) {
    dom = appDom4.removeNode(dom, newPageNode.id);
  }
  dom = appDom4.addFragment(dom, pageFragment, appRoot.id, "pages");
  return dom;
}
...
function extractPagesFromDom(dom) {
  const rootNode = appDom4.getApp(dom);
  const { pages: pageNodes = [] } = appDom4.getChildNodes(dom, rootNode);
  const pages = {};
  for (const pageNode of pageNodes) {
    pages[pageNode.name] = expandFromDom(pageNode, dom);
    dom = appDom4.removeNode(dom, pageNode.id);
  }
  return { pages, dom };
}
function extractThemeFromDom(dom) {
  const rootNode = appDom4.getApp(dom);
  const { themes: themeNodes = [] } = appDom4.getChildNodes(dom, rootNode);
...
function getSchemaUrl(obj) {
  return `https://raw.githubusercontent.com/mui/mui-toolpad/v${packageInfo_default.version}/docs/schemas/v1/definitions.json#properties/${obj}`;
}
...
var ToolpadProject = class {
  ...
  async applyDomDiff(domDiff) {
    await this.domAndFingerprintLock.use(async () => {
      ...
      const newDom = appDom4.applyDiff(dom, domDiff);
      return this.writeDomToDisk(newDom);
    });
  }
...
function* getPreferredPorts(port = DEFAULT_PORT) {
  while (true) {
    yield port;
    port += 1;
  }
...
async function createToolpadHandler({
  ...
  const publicPath = path8.resolve(currentDirectory3, "../../public");
  router.use(express5.static(publicPath, { index: false }));
...
async function startToolpadServer({ port, ...config2 }) {
  ...
  app.get("/health-check", (req, res) => {
    ...
    res.json({
      ...
      circleBuildNum,
      memoryUsage,
      memoryUsagePretty: mapValues2(memoryUsage, (usage) => prettyBytes(usage))
    });
  });
  const toolpadHandler = await createToolpadHandler(config2);
...
async function fetchAppUrl(appUrl) {
...
async function runEditor(appUrl) {
  let appRootUrl;
  try {
    appRootUrl = await fetchAppUrl(appUrl);
  } catch (err) {
    console.error(
      `${chalk5.red("error")} - No Toolpad Studio application found running under ${chalk5.cyan(appUrl)}
        Find more information about running a custom server at ${chalk5.cyan(
  ...
  console.log(
    `${chalk5.yellow("warn")}  - The editor command is deprecated and will be removed in the future, please visit ${chalk5.cyan(`${appRootUrl}/editor`)}`
  );
}
async function runApp({
  ...
  if (!port) {
    ...
    if (availablePort !== port) {
      console.error(`${chalk5.red("error")} - Port ${port} is not available. Aborted.`);
      process.exit(1);
    }
  }
  ...
  console.log(
    `${chalk5.green("ready")} - Toolpad Studio project ${chalk5.cyan(projectDir)} ready on ${chalk5.cyan(
      toolpadUrl
    )}`
  );
  if (dev) {
    try {
      ...
      console.error(`${chalk5.red("error")} - Failed to open browser: ${err.message}`);
    }
  }
  return {
    async dispose() {
...
// cli/index.ts
var currentDirectory4 = url5.fileURLToPath(new URL(".", import.meta.url));
async function runCommand(cmd, { dir, dev: toolpadDevMode, ...args }) {
  const projectDir = path9.resolve(process.cwd(), dir);
...
async function devCommand(args) {
  console.log(`${chalk6.blue("info")}  - starting Toolpad Studio application in dev mode...`);
  await runCommand("dev", args);
}
async function editorCommand({ dev: toolpadDevMode, ...args }) {
  await runEditor(args.url);
}
async function buildCommand({ dir, base }) {
  const projectDir = path9.resolve(process.cwd(), dir);
  console.log(`${chalk6.blue("info")}  - building Toolpad Studio application...`);
  const builderPath = path9.resolve(currentDirectory4, "./appBuilderWorker.mjs");
  ...
  console.log(`${chalk6.green("success")} - build done.`);
}
async function startCommand(args) {
  console.log(`${chalk6.blue("info")}  - Starting Toolpad Studio application...`);
...
async function cli(argv) {
  ...
  const parsedArgs = yargs(argv).scriptName("toolpad-studio").command(
    ["$0 [dir]", "dev [dir]"],
